...
# Larger sentences for single-language preview
BANGLA_SENTENCES = [
    "বাংলা ভাষার সৌন্দর্য ও সমৃদ্ধি",
    "প্রযুক্তির উৎকর্ষে নতুন সম্ভাবনা",
    "শিক্ষা, সংস্কৃতি ও ঐতিহ্যের ধারা",
    "বাংলা টাইপোগ্রাফি বিশ্বময় ছড়িয়ে পড়ুক",
    "সুন্দর ফন্ট আমাদের অভিব্যক্তি প্রকাশ করে"
]

ENGLISH_SENTENCES = [
    "The beauty of typography lies in its details",
    "Design is not just what it looks like, but how it works",
    "Creative minds shape the future with every stroke",
    "Minimalism brings clarity to complex ideas",
    "Elegance in design is achieved through simplicity"
]
...
# ---------- Font metadata parser (unchanged) ----------
...
# ---------- ORIGINAL PREVIEW (3-language random) ----------
@app.get("/preview")
def generate_preview(
    ...
    try:
        ...
        card1 = (60, 170, 1020, 480)
        card2 = (60, 510, 540, 820)
        card3 = (540, 510, 1020, 820)
...
# ---------- NEW ENDPOINT: SINGLE LANGUAGE PREVIEW ----------
@app.get("/preview_lang")
def preview_lang(
    font_url: str,
    font_name: str = "Font.ttf",
    requested_by: str = "User",
    ...
    try:
        ...
        header_font = get_hind_siliguri_font(40)
        sub_font = get_hind_siliguri_font(28)
        credit_font = get_hind_siliguri_font(30)

        # Title
        lang_label = "বাংলা" if lang == "bn" else "English"
        title_text = f"{lang_label} Preview — {detected_header_name}"
        title_bbox = draw.textbbox((0, 0), title_text, font=header_font)
        title_w = title_bbox[2] - title_bbox[0]
        draw.text(((width - title_w) / 2, 40), title_text, fill="#ffffff", font=header_font)
        draw.line([(80, 110), (width - 80, 110)], fill="#1e293b", width=2)
        ...
        # We'll display 3 sentences in a vertical stack with large size
        y_start = 150
        card_height = (height - y_start - 120) // 3  # 3 rows
        for i, sent in enumerate(sentences[:3]):
            y1 = y_start + i * (card_height + 10)
            y2 = y1 + card_height
            draw.rounded_rectangle([60, y1, width-60, y2], radius=16, fill="#131e30", outline="#1e293b", width=2)
            # Auto-fit font for this sentence inside the card
            max_w = width - 160
            max_h = card_height - 80
            card_font = get_auto_fit_font(custom_font_bytes, sent, max_width=max_w, max_height=max_h, start_size=90, min_size=24)
            bbox = draw.textbbox((0, 0), sent, font=card_font)
            tw, th = bbox[2] - bbox[0], bbox[3] - bbox[1]
            draw.text(((width - tw) / 2, y1 + (card_height - th) / 2 - 10), sent, fill="#f8fafc", font=card_font)

        # Footer
        draw.line([(80, height-100), (width-80, height-100)], fill="#1e293b", width=2)
        credit_text = f"Requested by: {requested_by}"
        cb = draw.textbbox((0, 0), credit_text, font=credit_font)
        cw = cb[2] - cb[0]
        draw.text(((width - cw) / 2, height-70), credit_text, fill="#38bdf8", font=credit_font)
